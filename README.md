...
# Умный ассистент
Ниже приведены сценарии использоавния. А вот [здесь](https://docs.google.com/presentation/d/1y_UjqVcAY6fXprVJ3wz4As17ROCDDo1KuwrupWiBL60/edit#slide=id.gb288c45447_5_27) доступен концепт безумного ассистента 
...
## Что под капотом?
Вы написали сообщение в поле вводе. Что происходит между прибытием запроса на сервер и отправке ответа? Генерацию ответа можно разбить на три этапа:
- Классификация
- Уточнение
- Действие

### __Классификация__
Первоначально необходимо понять, какое действие хочет выполнить пользователь. Для этого необходимо обучить классификатор. Была выбрана платформа [dialogflow](https://dialogflow.cloud.google.com), на которой можно обучить простенького чат-бота и интегрировать его в телеграм, вотсап, гуглоассистент, Алису и еще много куда! Было выбрано 11 сценариев работы(__*Intents*__) среди которых использовалось 6 сущностей(__*Entities*__). В каждом сценарии было от 3 до 30 примеров реплик, которые характеризуют данный сценарий. Пример:

![Alt-текст](./assets/readme/1.jpg)

В каждой сущности 5 до 40 примеров 

![Alt-текст](./assets/readme/2.jpg)

Сценарии сопоставлены с действиями, которые могут быть вызваны пользователям. Полный их список приведен ниже:

![Alt-текст](./assets/readme/3.jpg)

Список сущностей: 

![Alt-текст](./assets/readme/4.jpg)



Для взаимодействия с платформой использовалась [нативная библиотека](cloud.google.com/go/dialogflow/apiv2) от гугла. Итак, сообщение пользователя посылается гуглу, в ответ прилетает название текущего сценария и карта ключевых слов. Пример:
![Alt-текст](./assets/readme/5.jpg)

### __Уточнение__
Дальше необходимо на основе полученных ключевых слов, применить действие. Например, пользователь спрашивает "В каком году вышло производение Непутёвый ученик в школе магии: Гость?". Гугл распознает, что событие: `Узнать год выпуска`, имя тайтла: `Непутёвый ученик в школе магии: Гость?`. Сервер обратится к БД, чтобы найти среди тайтлов тот самый. Отлично, всё просто и прекрасно работает. Но что будет, если пользователь сформулирует вопрос чуточку иначе? Например "Кагда вышла первая серия Непутёвого учиника в академии магии". Гугл посчитает, что название тайтла `Непутёвого учиника в академии магиии`. Поиск по такому названию ничего не даст потому что:
- Название не в именительном падеже
- Слово "школа" заменено на "академия"
- В словах "ученика" и "магии" допущены ошибки

### __Stemming__

Для того, чтобы устранить все эти проблемы, название тайтла подвергается предварительной обработке. Каждое слово приводится к именительной форме(__stemming__) c помощью [snowball](github.com/kljensen/snowball). Так, "Непутёвого учиника в академии магиии" превратится "Непутёв учиник в академ маг". 

Следующий шаг - удаление "лишних" слов из имени. Данное действие применяется по отношению к словам, написанным на кириллице, поскольку чат-бот умеет общаться только на русском. Иногда гугл неправильно распознает начало название, добавляя к нему вспомогательные слова, такие как "аниме", "сериал", "и" и т.д. Плюс нам также не нужны предлоги в названии, поскольку они не несут полезной смысловой нагрузки, избавимся и от них. Так мы получим "Непутёв учиник академ маг". 

### __Fuzzy search__

Дальше происходит размытый поиск(__fuzzy__) - поиск совпадения подстрок в запрошиваемом имени и именах из БД. Причем совпадением считается, если совпадает целиком слово или большая его часть. Делается это с помощью [fuzzy](https://github.com/sahilm/fuzzy) Так, при сравнении "Непутёвый ученик в школе магии: Гость?" из БД найдёт совпадение с "Непутёв учиник академ маг" по частям "Непутёв", "академ" "маг". 

### __Расстояние Левенштейна__

Отлично, всё работает? Для данного примера да. А что если пользователь запросит аниме "Гинтама", назвав его "Гентома"? Тут то поиск по шаблонам ничего не даст. На этот случай запускается поиск похожих слов по расстоянию Левенштейна.Его реализация взята из [closestmatch](https://github.com/schollz/closestmatch) - либы, основанной на какой то другом алгоритме, но работающей настолько плохо(она предназначена не для поиска по словам, а для поиска строк по тексту), что применять, что применять ее не было смысла. Зато они написали самосного Левенштейна для тестов, вот он и применен здесь. 

Итак, если расстояние Левенштейна меньше 5, то найденное название считается похожим. Почему именно 5? Получено эмпирическим путем. Очевидно, что необходимо продолжать опыты. Были предприняты использовать динамическое число(в зависимости от длины слова), но это давало плохие результаты в случае, если пользователь забыл или менял слова. Например тайтл с названием "Дальше чем космос". Если юзер укажет "Далее за космос" поиск по шаблонам поймёт, что слово похожее, а вот поиск по расстоянию Левенштейна ничего найдет(если повысить минимальную дистанцию, то найдет не то). А если юзер напишет с ошибками  "Далии зо коснос", то уже и поиск по шаблонам сломается. 

## __Действие__

Итак, действие выбрано, тайтл найден, ключевые слова найдены. Можно применять действие? Можно. Возможные действия: 
- Узнать "что-то"... Это описание разных характеристик аниме - обычный запрос к полям данных, ничего интересного, двигаемся дальше
- Действие "оценка" - ставит оценку тайтлу, название которого польователь указал в текущем или прошлых запросах. В прошлых запросах? Именно! Если в каком то запросе упоминается название тайтла, то оно сохраняется в памяти и используется как контекст для остальных действий. Так, если юзер не указал название тайтла в текущем, а оно необходимо для выполнения действия, то будет использовано то, которое хранится в контексте.
- Действие "рекомендации по жанрам и названиям" - ищет тайтлы похожие на те, что предложил юзер((__контентная РС__)), а полученные результаты фильтруют в рамках искомых жанров.
- Действие "рекомендации по моим предпочтения" - ищет тайтлы похожие на те, которым вы поставили оценки. В отличии от прошлого действия здесь учитываются баллы, которые вы им поставили. Если вы поставили тайтлу низкий балл, то похожих тайтлов точно не будет. А если высокий балл, то система будет стараться найти что то похожее
- Действие "рекомендации похожего" - ищёт тайтлы, похожие на тот тайтл, который вы только что обсуждали с ботом.